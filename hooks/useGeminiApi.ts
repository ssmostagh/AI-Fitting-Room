...
const getAiClient = (): GoogleGenAI => {
  ...
  // Prioritize VITE_GEMINI_API_KEY (Standard), fallback to legacy VITE_API_KEY
  const apiKey = window.env?.VITE_GEMINI_API_KEY ||
    window.env?.VITE_API_KEY ||
    import.meta.env.VITE_GEMINI_API_KEY ||
    import.meta.env.VITE_API_KEY;

  if (!apiKey) {
    console.error("VITE_GEMINI_API_KEY is missing. Please set it in .env or Cloud Run environment variables.");
    throw new Error("VITE_GEMINI_API_KEY is missing. Please check your .env file or deployment configuration.");
  }
